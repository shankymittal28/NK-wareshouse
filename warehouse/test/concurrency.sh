...
Q "select t.act_as('aaaaaaaa-0000-0000-0000-000000000002'); select wh.record_opening('cccccccc-0000-0000-0000-000000000001', 100, now() - interval '3 days');" >/dev/null
...
for i in $(seq 1 8); do
  psql -X -q --no-psqlrc -t -A -d "$DB" \
    -c "select t.act_as('aaaaaaaa-0000-0000-0000-000000000002');" \
    -c "select wh.submit_event(\$json\$$DOC\$json\$::jsonb, 0)" >/dev/null 2>&1 &
done
wait
...
for pass in 1 2; do
  Q "select t.act_as('aaaaaaaa-0000-0000-0000-000000000002');
