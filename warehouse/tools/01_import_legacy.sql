...
keyed as (
  select wh_import.source_key(category, brand, thickness, size, design) as source_key,
         ...
         (nullif(btrim(brand),'') is null
          or (category_code='Plywood' and nullif(btrim(thickness),'') is null)
          or nullif(btrim(size),'') is null
          or (category_code='Doors' and nullif(btrim(design),'') is null)) as needs_naming,
         qty, direction
    from rows
),
agg as (
  select source_key, category_code, (array_agg(attrs))[1] as attrs, bool_or(needs_naming) as needs_naming,
...
select a.source_key, a.category_code, a.attrs,
       ...
       a.needs_naming, a.src_rows, a.src_net
  from agg a;
...
insert into wh.material(material_id, category_code, attrs, origin, needs_naming)
select im.material_id, im.category_code, im.attrs, 'legacy_import', im.needs_naming
